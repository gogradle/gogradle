...
task golint(type: com.github.blindpirate.gogradle.Go) {
    dependsOn goVendor // make this task depend on goVendor task
...
The `build` and `test` are two independent dependency sets. Java developer should be familiar with the concept. By default, `goBuild` dependes on `resolveBuildDependencies`, thus when `goBuild` runs, only `build` dependencies exist in `vendor`; `goTest` depends on both `resolveBuildDependencies` and `resolveTestDependencies`, thus when `goTest` runs, both `build` and `test` dependencies exist in `vendor`.
